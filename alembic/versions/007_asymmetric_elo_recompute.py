"""Retroactive ELO recompute with asymmetric win/loss formula.
...
asymmetric formula: the winning side gains the full margin-aware delta while
the losing side only loses delta * ELO_LOSS_FACTOR (default 0.5).  This
rewards regular participation — a one-off winner banks a single win, while
players with many matches accumulate rating over time.

    mult     = log(1 + |net_games_A|)
    delta    = K * mult * (S_a - E_a)         raw symmetric delta for team A
    winner   gains |delta|
    loser    loses |delta| * loss_factor

Rewrites:
  - league_members.rating (current ratings)
  - matches.elo_delta (still stores the raw symmetric delta)
...
ELO_LOSS_FACTOR: float = float(os.environ.get("ELO_LOSS_FACTOR", "0.5"))
...
def _split_team_changes(delta: float) -> tuple[float, float]:
    """(change_a, change_b): the losing side's deduction is dampened."""
    if delta >= 0:
        return delta, -delta * ELO_LOSS_FACTOR
    return delta * ELO_LOSS_FACTOR, -delta
    ...
    for league_id in league_ids:
        ...
        for season_id, is_current in seasons:
            ...
            for (match_id,) in matches:
                ...
                change_a, change_b = _split_team_changes(delta)

                # 6. Apply asymmetric rating changes
                for uid, _ in team_a:
                    bind.execute(
                        sa.text(
                        ...
                        {"d": change_a, "lid": league_id, "uid": uid},
                    )
                for uid, _ in team_b:
                    bind.execute(
                        sa.text(
                        ...
                        {"d": change_b, "lid": league_id, "uid": uid},
                    )
